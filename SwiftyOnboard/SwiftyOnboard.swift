...
public class SwiftyOnboard: UIView, UIScrollViewDelegate {
    ...
    open func goToPage(index: Int, animated: Bool) {
        if index < self.pageCount {
            ...
            let xOffset : CGFloat = CGFloat(index) * self.containerView.frame.width
            self.containerView.setContentOffset(CGPoint(x: xOffset, y: self.containerView.contentOffset.y), animated: animated)
        }
    }
}
